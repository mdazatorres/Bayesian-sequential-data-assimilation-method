...
class covid_mcmc2(fm_matrix):
    def __init__(self, Region, init_day, N, data_fnam, out_fnam, Pobs_I, Pobs_D, init_index,
                 init, init_index0, init0, intervention_day, exit_probs, R_rates, trim, court, size_window, num_data,delta_day, workdir="./../"):
        ...
        super().__init__(m=self.data.shape[0], intervention_day=intervention_day, exit_probs=exit_probs, R_rates=R_rates)

        if isinstance(Pobs_I, float):
            self.Pobs_I = Pobs_I
            self.Pobs_D = Pobs_D
        else:  # Nowcasting function, forming different obs probabilities
            self.Pobs_I = Pobs_I(np.arange(self.m+1, 1, -1))
            self.Pobs_D = Pobs_D(np.arange(self.m+1, 1, -1))
            
        self.num_betas = 2
        self.solve_plain = self.solve_plain1
        self.w = (np.arange(0, self.m) + 1.0) ** -(0.3)
    ...
    def lprior(self, p):
        ...
        if self.court == 0:
            ...
            for i in range(1, self.num_betas):
                log_prior += np.sum(ss.lognorm.logpdf(p[self.init_m + i], 1.0, scale=p[self.init_m + i - 1]))  # scale=np.exp(log(p[i-1]))
            ...
            for i in range(self.num_betas):
                mu, std = self.post_params['beta' + str(i) + str(self.court - 1)]  # beta
                log_prior += np.sum(ss.norm.logpdf(p[self.init_m + i], mu, scale=std))
    ...
    def sim_init(self):
        ...
        if self.court == 0:
            ...
            for i in range(self.num_betas):
                p[self.init_m + i] = np.random.uniform(low=0.01, high=5.0)   # beta
            ...
            for i in range(self.num_betas):
                mu, std = self.post_params['beta' + str(i) + str(self.court - 1)]  # beta
                p[self.init_m + i] = ss.norm.rvs(mu, scale=std)
    ...
    def fit_posterior(self):
        ...
        for i in range(self.num_betas):
            Beta = self.samples[:, self.init_m + i]
            self.post_params['beta' + str(i) + str(self.court)] = ss.norm.fit(Beta)
    ...
    def PlotEvolution(self, pred, ty=0, cumm=False, log=False, ax=None,\
        ...
        if ty == 0:
            ...
            title = 'Casos confirmados diarios'
        else:
        ...
        if cumm:
            ...
            ylabel = 'Num. de casos acumulados'
        ...
        if log:
            # #plot data
            if label_cases:
                ax.semilogy(days[:length], prevalence, 'k*', lw=1, label='Casos')
            else:
                ax.semilogy(days[:length], prevalence, 'k*', lw=1)
            if self.trim < 0:
                if label_cases:
                    ax.semilogy(days[length:(length-self.trim)], future, 'r*', lw=1, label='Casos futuros')
                else:
                    ax.semilogy(days[length:(length-self.trim)], future, 'r*', lw=1)
            ylabel += ' (log)' 
        else:
            if label_cases:
    ...
    def PlotEvolution1(self, pred, solns, court, ty=0, cumm=False, log=False, ax=None,
        ...
        if cumm:
            ...
            ylabel = 'Num. accumulated cases'
            title = 'Accumulated cases ' + title
        else:
            prevalence = data  # aggregate observed data
        ...
        if log:
            ...
            if label:
                ax.plot(days[:shift], sv[:shift, 8], '-', linewidth=2, color=color, label='Mediana')
            else:
        ...
        if plotdata:
            ...
            if log:
                ax.semilogy(days_all, prevalence_all, 'k*', lw=1, label='Casos')
            else:
    ...
    def Plot_evol_params(self, index, court, q, ax):
        for i in range(court + 1):
            if i == court:
                beta = self.samples[:, self.init_m + index]
            else:
                samples = pickle.load(
                    open(self.workdir + 'output/' + self.out_fnam + 'court_' + str(i) + '_samples.pkl', 'rb'))
                beta = samples[:, self.init_m + index]

            h = np.quantile(beta, q=np.array(q) / 100)
            ax.bar(x=i, bottom=h[0], height=h[4] - h[0], width=0.9, color='blue', alpha=0.25)
            ax.bar(x=i, bottom=h[1], height=h[3] - h[1], width=0.9, color='blue', alpha=0.25)
            ax.hlines(y=h[2], xmin=i - 0.9 / 2, xmax=i + 0.9 / 2, color='red')
            ax.tick_params(which='major', axis='x', labelsize=10, labelrotation=0)
        ax.set_xticks(range(court + 1))

        if index == 0 or index == 1:
            ax.set_ylabel(r"$\beta %s$" % index)
            ax.set_title(r"%s $\beta %s$" % (self.Region, index))
        elif index == 2:
